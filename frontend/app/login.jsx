  ...
  async function handleLogin() {
    try {
      ...
      if (data.jwt && data.username) {
        login(data.jwt, data.username); // do we need await?
        // fixes the issue where Expo Router fails to close the current modal when running router.push("") 
        // https://github.com/expo/expo/issues/26922#issuecomment-1996862878
        router.dismissAll(); // navigate to first screen within the lowest stack??
        setTimeout(() => {
          router.push("/");
        }, 0);
        // alternatively use router.back() if possible, but wont work here since we dont want to go back to create account
      }
    } catch (error) {
