...
/*   @Autowired
  ...
  SecurityFilterChain securityFilterChain(HttpSecurity http) throws Exception {
    ...
    .headers(headers -> headers
        .frameOptions(frameOptions -> frameOptions.disable()) // Disable X-Frame-Options to allow H2 console to be displayed in a frame.. otherwise it just says localhost is blocked
    )
    ...
    .formLogin(form -> form
            .disable() // Disable form-based login.. otherwise forces reroute to http://localhost:8080/login if no access..?
    )
    .httpBasic(httpBasic -> httpBasic
        .disable() // Disable basic HTTP authentication.. otherwise forces reroute to http://localhost:8080/login if no access..?
    )
